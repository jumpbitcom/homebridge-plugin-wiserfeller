...
export class Motor {
  ...
  constructor(
    ...
    this.platform.myClient?.loadStateChange.on(
  ...
  async setTargetPosition(value: CharacteristicValue): Promise<void> {
    ...
    if ('number' === typeof value) {
      ...
      return this.platform.myClient?.setLoadState(this.accessory.context.load.id,
        {
  ...
  async updateState(loadState: IloadState) {
  ...
  async setHoldPosition(): Promise<void> {
    this.platform.myClient?.ctrlLoad(this.accessory.context.load.id, {
      button: 'stop',
      event: 'click',
    });
  }
